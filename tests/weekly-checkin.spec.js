...
test.describe('Weekly check-in on Today', () => {
  ...
  test('release and storage metadata stay unchanged apart from version 1.6.3', async ({ page }) => {
    await openOnTrackingDay(page, 7);
    expect(await page.evaluate(() => ({ version: VERSION, schema: DATA_VERSION, key: KEY, recovery: RECOVERY_KEY })))
      .toEqual({ version: '1.6.3', schema: 6, key: 'ketoProtokoll_v1', recovery: 'ketoProtokoll_recovery_v1' });
  });
});
